...
#  the five we subtract it making four. The same principle applies to 
...
# 
# Solution:
# 就一种判断情况，如果后者比前者大，则一次性读两个，后者减前者
###########################################################
class Solution:
    def romanToInt(self, s: 'str') -> 'int':
        que = ["M","D","C","L","X","V","I"]
        lit = { "M":1000,"D":500,"C":100,"L":50 ,"X":10,"V":5,"I":1}
        current = 0
        r_value = 0
        while current != len(s):
            position = que.index(s[current])
            if current+1<len(s) and s[current+1] not in que[position:]:
                r_value += lit[s[current+1]]-lit[s[current]]
                current += 1
            else:
                r_value += lit[s[current]]
            current += 1
        return r_value
...
if __name__ == "__main__":
    ...
    for each in s:
        print("Input:"+each)
        print("Output:"+str(app.romanToInt(each)))