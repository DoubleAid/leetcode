...
class Solution:
    def myAtoi(self, str):
        isNegative = 1
        strn = str.lstrip() #去除字符串左边的空格
        if strn=="":
            return 0
        #如果第一个字符不是符号也不是数字
        if strn[0].isdecimal()!=True and strn[0]!='-' and strn[0]!="+":
            return 0
        count = 0
        if strn[0].isdecimal()!=True:
            if strn[0]=='-':
                isNegative = -1
            count += 1
        num = 0
        while len(strn) > count and strn[count].isdigit():
            num = num*10 + int(strn[count])
            count+=1
        num *=isNegative
        num = num if num>-pow(2,31) else -pow(2,31)
        num = num if num<pow(2,31)-1 else pow(2,31)-1
        return num
